...
export function buildDigestEmail({ dueSoon, underfunded }) {
  const sections = [];
  ...
  const subject = `Sinker: ${subjectBits.join(", ")}`;

  const html = `
    <div style="font-family: -apple-system, sans-serif; color:#2b2420; max-width:480px;">
      <p style="font-size:20px; font-weight:700; color:#d95926; margin:0 0 12px;">Sinker</p>
      ${sections.join("")}
      <p style="font-size:12px; color:#6b5f54; margin-top:28px; border-top:1px solid #f1e5da; padding-top:12px;">
        You're getting this because email reminders are turned on in your Sinker settings. You can turn them off any time from the Settings page.
      </p>
    </div>
  `;
