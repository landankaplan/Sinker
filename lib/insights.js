...
// Rolls a flat contribution list up into the handful of headline numbers
// the Insights page shows as stat tiles.
//
// Formula (plain English):
//   totalAllTime = sum of every contribution's amount, no date filter.
//   totalThisMonth = sum of contributions whose local calendar date falls
//     in the same month AND year as `today`.
//   totalThisYear = sum of contributions whose local calendar date falls
//     in the same year as `today`.
//   contributionCount = how many contribution rows were summed into
//     totalAllTime (i.e. how many "Add contribution" actions ever happened).
//   averageContribution = totalAllTime / contributionCount (0 if none).
//
// Worked example (today = March 15, 2026):
//   Contributions: $50 on Jan 3 2026, $20 on Mar 1 2026, $10 on Mar 15 2026,
//   $100 on Dec 20 2025.
//   totalAllTime = 50 + 20 + 10 + 100 = 180
//   totalThisMonth (March 2026) = 20 + 10 = 30
//   totalThisYear (2026) = 50 + 20 + 10 = 80
//   contributionCount = 4
//   averageContribution = 180 / 4 = 45
//
// Edge cases:
//  - No contributions at all: every total is 0, averageContribution is 0
//    (not NaN/Infinity - division is guarded).
//  - A contribution with an unparseable date is excluded from all three
//    sums and from contributionCount, same as buildCumulativeSavingsSeries.
//
// @param {Array<{amount: number, contributed_at: string|Date}>} contributions
// @param {Date} [today] defaults to now
// @returns {{ totalAllTime: number, totalThisMonth: number, totalThisYear: number, contributionCount: number, averageContribution: number }}
export function summarizeContributions(contributions, today = new Date()) {
  const valid = (contributions || [])
    .map((c) => ({ date: toLocalMidnight(c.contributed_at), amount: Number(c.amount) }))
    .filter((c) => !Number.isNaN(c.date.getTime()) && !Number.isNaN(c.amount));
  ...
  for (const c of valid) {
    totalAllTime += c.amount;
    if (c.date.getFullYear() === thisYear) {
      totalThisYear += c.amount;
      if (c.date.getMonth() === thisMonth) totalThisMonth += c.amount;
    }
  }

  const contributionCount = valid.length;
  const averageContribution = contributionCount > 0 ? totalAllTime / contributionCount : 0;

  return { totalAllTime, totalThisMonth, totalThisYear, contributionCount, averageContribution };
}

// How long, on average, a completed fund took from creation to completion -
// a rough "how long does it actually take me to hit a savings goal" number.
//
// Formula (plain English):
//   For each fund that has a completed_at, days_to_complete = the whole
//     number of days between created_at and completed_at (rounded, since
//     both are real timestamps and a fractional day isn't meaningful here).
//   averageDays = the mean of days_to_complete across all completed funds.
//
// Worked example:
//   Fund A: created Jan 1, completed Jan 31 -> 30 days.
//   Fund B: created Feb 1, completed Feb 11 -> 10 days.
//   averageDays = (30 + 10) / 2 = 20
//
// Edge cases:
//  - No completed funds yet: returns { applicable: false, averageDays: 0,
//    completedCount: 0 } rather than NaN - callers should check
//    `applicable` before displaying this stat, the same pattern used by
//    calculateBehindPace/calculateShortfallProjection in lib/calculations.js.
//  - A fund completed on the same day it was created counts as 0 days
//    (not excluded) - that's a real, valid data point, not an error.
//
// @param {Array<{created_at: string, completed_at: string|null}>} funds
// @returns {{ applicable: boolean, averageDays: number, completedCount: number }}
export function calculateAverageTimeToComplete(funds) {
  const durations = (funds || [])
    .filter((f) => f.completed_at)
    .map((f) => {
      const created = new Date(f.created_at).getTime();
      const completed = new Date(f.completed_at).getTime();
      return Math.round((completed - created) / MS_PER_DAY);
    })
    .filter((d) => !Number.isNaN(d));

  if (durations.length === 0) {
    return { applicable: false, averageDays: 0, completedCount: 0 };
  }

  const averageDays = durations.reduce((sum, d) => sum + d, 0) / durations.length;
  return { applicable: true, averageDays, completedCount: durations.length };
}
